...
if filter.gate_flag
    for tabidx=1:length(glmb_predict.tt)
        glmb_predict.tt{tabidx}.gatemeas= gate_meas_ekf_idx(meas.Z{k},filter.gamma,model,glmb_predict.tt{tabidx}.m,glmb_predict.tt{tabidx}.P);
    end
else
    for tabidx=1:length(glmb_predict.tt)
...
for tabidx= 1:length(glmb_predict.tt)
    for emm= glmb_predict.tt{tabidx}.gatemeas
            ...
            [qz_temp,m_temp,P_temp] = ekf_update_multiple(meas.Z{k}(:,emm),model,glmb_predict.tt{tabidx}.m,glmb_predict.tt{tabidx}.P);   %kalman update for this track and this measurement
...
for t=1:length(est.T)
    ...
    for u=1:length(tah)
        ...
        emm= tah(u);
        if emm > 0
            [qz,m,P] = ekf_update_multiple(meas.Z{k}(:,emm),model,m,P);
            w= qz.*w+eps;
            w= w/sum(w);
        end
