...
filter.npt= 5000;                   %number of particles per track
filter.nth= 2000;                   %threshold on effective number of particles before resampling
...
if filter.gate_flag
    for tabidx=1:length(glmb_predict.tt)
        glmb_predict.tt{tabidx}.gatemeas= gate_meas_smc_idx(meas.Z{k},filter.P_G,model,glmb_predict.tt{tabidx}.x,glmb_predict.tt{tabidx}.w,filter.gate_ngrid);
    end
else
    for tabidx=1:length(glmb_predict.tt)
...
for tabidx=1:length(glmb_update.tt)
    avps(model.T_birth+tabidx)= glmb_update.tt{tabidx}.w(:)'*compute_pS(model,glmb_update.tt{tabidx}.x)+eps(0);
end
avqs= 1-avps;
...
for tabidx= 1:length(glmb_predict.tt)
    for emm= glmb_predict.tt{tabidx}.gatemeas
            ...
            w_temp= avps(tabidx)*compute_pD(model,glmb_predict.tt{tabidx}.x).*glmb_predict.tt{tabidx}.w(:).*compute_likelihood(model,meas.Z{k}(:,emm),glmb_predict.tt{tabidx}.x)'; x_temp= glmb_predict.tt{tabidx}.x;  %weight update for this track and this measuremnent      
...
for t=1:length(est.T)
    ...
    w= ones(est.filter.npt,1)/est.filter.npt; 
    x= gen_gms(model.w_birth{bidx},model.m_birth{bidx},model.P_birth{bidx},est.filter.npt);
    for u=1:length(tah)
        x= gen_newstate_fn(model,x,'noise');
        k= ks+u-1;
        emm= tah(u);
        if emm > 0
            qz = compute_likelihood(model,meas.Z{k}(:,emm),x)';
            w= qz.*w+eps;
            w= w/sum(w);
        end
