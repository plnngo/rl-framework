...
for tabsidx=1:length(tt_lmb_update)
    ...
    [mtemp_predict,Ptemp_predict]= ekf_predict_multiple(model,tt_lmb_update{tabsidx}.m,tt_lmb_update{tabsidx}.P);       %kalman prediction for GM
...
if filter.gate_flag
    for tabidx=1:length(tt_predict)
        tt_predict{tabidx}.gatemeas= gate_meas_ekf_idx(meas.Z{k},filter.gamma,model,tt_predict{tabidx}.m,tt_predict{tabidx}.P);  
    end
else
    for tabidx=1:length(tt_predict)
...
for tabidx= 1:length(tt_predict)
    for emm= tt_predict{tabidx}.gatemeas
            ...
            [qz_temp,m_temp,P_temp] = ekf_update_multiple(meas.Z{k}(:,emm),model,tt_predict{tabidx}.m,tt_predict{tabidx}.P);    %kalman update for this track and this measurement
